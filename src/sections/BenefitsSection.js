...
function BenefitsSection() {
  const benefits = [
    {
      title: '100% Organic',
      icon: <svg className="text-primary" fill="currentColor" width="32" height="32" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="row row-cols-1 row-cols-md-3 g-4">
        {benefits.map((benefit, index) => (
          <div key={index} className="col">
            <div className="d-flex align-items-center gap-3 rounded-lg border border-2 p-4 bg-white">
              {benefit.icon}
              <h3 className="fs-5 fw-bold">{benefit.title}</h3>
            </div>
          </div>
        ))}
      </div>
